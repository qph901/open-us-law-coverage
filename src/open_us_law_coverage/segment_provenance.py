...
**2. `FR_*` distinct-text collisions are best explained as co-numbered distinct
documents, not ordered segments — enough to forbid concatenation, not enough to
call every row a self-contained capture.** M0.5A read the 165,044 distinct-text
`FR` groups as "one Federal Register document split into multiple text segments."
The single-snapshot evidence weighs **against** that reading: their rows are
**physically scattered** (essentially none are an adjacent block), the
segment-relationship sample finds a **continuation rate of essentially zero** (no
seam begins lowercase, the proxy for a mid-sentence continuation), and **almost
every group's rows restart with the same agency preamble**. The safe, load-bearing
conclusion is therefore **negative**: there is no evidence of an ordered
single-document segmentation, so concatenating the rows "in order" reconstructs
nothing and must not be attempted. The stronger positive claim — that each row is a
*self-contained* Federal-Register document — is **supported but not established**:
physical scattering does not *logically* disprove ordered segmentation (an upstream
writer could interleave groups), the continuation proxy is a coarse lowercase test,
and the sample is lexically biased (see the methodology caveat below). `CFR_*` is
genuinely mixed (a minority of seams begin lowercase), i.e. some real
section-segmentation plus duplicates. The corrected identity consequence is below.
...
## Methodology caveats (bounding the mechanism claim)

The **non-concatenation decision is safe** and rests only on the negative evidence
above. The mechanism explanation ("each row is a self-contained capture") is
**supported, not established**, and two sampling limits keep it there — to be
lifted before that positive claim is promoted:

- **The distinct-text sample is lexically biased.** Groups are taken as the first
  `SAMPLE_GROUPS` per namespace ordered by `act_id`, which skews toward the
  lexically first document types (e.g. `FR_PRORULE`). A stratified or random sample
  across document types is future work.
- **The continuation detector is coarse.** "Next row begins lowercase" is a weak
  proxy for a mid-sentence seam; a structural detector (sentence boundaries,
  dangling enumerations, cross-row bracket/quote balance) would test segmentation
  far more strongly. Until then, low measured continuation bounds the claim rather
  than proving self-containment.
"""
