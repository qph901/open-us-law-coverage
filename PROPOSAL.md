...
SourceDocumentAssembly                # content-addressed by its physical members
  ...
  evidence[]
AssemblyIdentityAssociation           # SEPARATE, versioned; the key is NOT hashed into the assembly
  (DerivedArtifactProvenance)         # inputs = exactly [assembly edge -> the body it links];
                                      #   producer_name/_version = the identity strategy;
                                      #   config_hash folds in (source_identity_key, legal_id)
  source_identity_key                 # the (mutable) identity-strategy key — body, never an edge
  assembly_artifact_id                # -> the immutable assembly above
  legal_id                            # attaches here
  payload_hash                        # semantic content address (D2) — the tripwire covers it
  evidence[]
```
...
- **The assembly is content-addressed by its physical members, and carries no `source_identity_key`** (M1A.5 review B3). An unhashed identity key on a content-addressed object would let two different bodies share one `artifact_id`. The mutable key — and `legal_id` — live on a **separate versioned `AssemblyIdentityAssociation`** that may be re-emitted when an identity strategy improves *without* creating two assembly bodies under one id. So `legal_id` still attaches at the assembly level (M0.5A.1 disproved 1:1 source-to-document), just via that association rather than as a field on the immutable artifact. **That association is itself a first-class derived artifact** (M1A.5 A.2/A.3): it carries its own `DerivedArtifactProvenance` — exactly one `assembly` edge to the body it links — and a validated `payload_hash`, so the D2 collision tripwire covers it like every other artifact. The mutable key is deliberately **not** a provenance edge (the durable-FK rule); it is folded into the derivation `config_hash`, so key A and key B over one assembly get two distinct association `artifact_id`s instead of colliding, while still pointing at the **same** unchanged `assembly_artifact_id`.
...
Assert: identity strategy v1 (key A) and v2 (key B) coexist over the same records; downstream provenance referencing those records stays valid; no immutable artifact is keyed by `source_identity_key`. Because the key lives only on the separate `AssemblyIdentityAssociation`, key A and key B associate with the **same** assembly `artifact_id` without changing the assembly body. Extend the same coexistence test to assembly v1/v2 over the same members, and add the D1 recompute-frontier property: **a membership change re-hashes the `SourceIdentityGroup` and every affected `SourceIdentityMemberAnnotation`.** The durable-FK suite graduates from fabricated annotations to **actual producer outputs** once the concrete strategies exist (M1A.5 B.3). The version boundary is exercised as a genuine **v1/v2 of one strategy** — the real `cfr_identity_v1` producer (v1, `provisional`) against an explicit v2 fixture of the *same* producer name making a real rule change (`resolved`) — not two different namespace strategies standing in for a version bump. The v2 side is honestly labeled a fixture: no second version of any concrete strategy has shipped yet, so it exists only to cross the boundary the shipped code cannot.
...
### M1A.5 — shared derived-artifact foundation *(CLOSED)*
Build the shared derived-artifact contracts once, so every downstream annotation inherits provenance and the durable-FK discipline instead of re-inventing them. **Closed 2026-08-28: the contracts were corrected and frozen (Phase A), the concrete producers built on them (Phase B), and the evidence recommissioned (Phase C) — in that order (D3), because the shapes had persisted no artifacts, so correcting them was a design edit, not a migration.** Deliverables:
...
### Residual deltas resolved for M1A.5 closure (2026-08-28)

A second review pass settled four residual deltas. The milestone was **M1A.5 closure**: close the derived-layer contracts, *then* build the concrete `SourceIdentityStrategy` producers on the corrected contracts, *then* recommission the evidence — **no CFR assembly and no M1B freeze until done.** The settled decisions are folded into the contracts above.
