...
- **`SourceIdentityGroup` (real producer)** — single-member groups: **161,566**, multi-member: 0 | `state_statute_act_id_v1`:161,566 | status `resolved`:161,566
- **`duplicate_row` within each identity group** (real `detect_duplicate_rows`): **0** flagged (a single-member group has no sibling to duplicate — the contrast below is the point)
...
- `SourceDocumentAssembly` losslessness (trivial, **full corpus** — 161,566 rows): PASS
